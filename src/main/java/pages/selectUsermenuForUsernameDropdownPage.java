...
//	public WebElement mySettingsConformationMessage;
	...
	String mainWindowId=BaseSteps.parentWindowId(BaseSteps.getDriver());
	
	public boolean verifyLogOut(WebDriver driver,String loginTitle) {
		boolean isLogoutSuccessfull=false;
		if(WaitUtils.waitForElementToBeVisible(driver, logoutOptionFromUserMenu)) {
	...
	//reportsTab
	public boolean verifyReportsAddedToSalesPage(WebDriver driver) {
		boolean isReportsTabAvailable=false;
		if(WaitUtils.waitForElementToBeVisible(driver,salesPageOnContentTab)) {
			salesPageOnContentTab.click();
			if(WaitUtils.waitForElementToBeVisible(driver, salesOption)) {
				salesOption.click();
			}
			if(WaitUtils.waitForElementToBeVisible(driver, moreTabs)) {
				moreTabs.click();
				if(reportsTab.isDisplayed()) {
					isReportsTabAvailable=true;
					logger.info("Reports field is added in Sales Page" );
				}else {
					 logger.error("Reports field is not visible in Sales Page" );
				 }
			}
		}
		return isReportsTabAvailable;
...
//			BaseSteps.selectOption(selectcustomApp, selectCustomOption);//"Salesforce Chatter"
	...
	public boolean verifyLoginHistory(WebDriver driver) {
		...
		if(WaitUtils.waitForElementToBeVisible(driver, mySettingsPersonalLink)) {
			...
			if(WaitUtils.waitForElementToBeVisible(driver, personalHistoryLink)) {
				personalHistoryLink.click();
				logger.info("PASS:LoginHistory is Displayed");
	...
	public boolean verifyPhotoUpload(WebDriver driver,String imagefilepath) {
		...
		if(moderatorLink.isDisplayed()) {
			BaseSteps.mouseHover(driver, moderatorLink);
			if(updatePhoto.isDisplayed()) {
			    ...
			    BaseSteps.cropPhotoClickNHold(driver,photoCrop);
...
//		}
	...
	public boolean selectUserMenuOption(WebDriver driver,String option) {
		...
		if (WaitUtils.waitForElementToBeVisible(driver,userMenuOption)) {
			...
			if(actualProfilePage.contains("Swapna Abcd")) {
				logger.debug("PASS: User profile page is displayed");
			}else {
				logger.error("User profile page not displayed");
			}
	...
	if(userMenu.isDisplayed()) {
  	  if(BaseSteps.mouseHover(driver,userMenu)) {
