...
public class ContactsPage extends BasePage{
	...
	public List<WebElement> detailContactPage;
	
	public void verifyClickedContactPageDetails(WebDriver driver) {
		if(WaitUtils.waitForListOfElementToBeVisible(driver, detailContactPage)) {
			for(WebElement detailedPageTable:detailContactPage) {
				System.out.println(detailedPageTable.getText()+" ");
			}
		}
	}
	...
	public boolean verifyMyContactsViewDisplayedInDropDown(WebDriver driver) throws IOException {
		...
		if(WaitUtils.waitForElementToBeVisible(driver, contactDropDown)) {
			if(BaseTest.selectOption(contactDropDown,DataUtils.readContactsPageData("contact.selectMyContactsDropDownOption"))){
	...
	@FindBy(xpath="//div[@class='pbBody']/table/tbody/tr")
	public List<WebElement> tablebodyOfMyContacts;
	
	public void verifyMycontactsCreatedDetails(WebDriver driver) {
		
		if(WaitUtils.waitForListOfElementToBeVisible(driver, tablebodyOfMyContacts)) {
			for(WebElement eachContactInMyContacts:tablebodyOfMyContacts) {
				System.out.println(eachContactInMyContacts.getText()+" ");
			}
		}
		}
	...
	public boolean verifyRecentlyCreatedContacts(WebDriver driver) throws IOException {
		...
		if(WaitUtils.waitForElementToBeVisible(driver, CreatedOption)) {
			if(BaseTest.selectOption(CreatedOption, DataUtils.readContactsPageData("contact.recentlyCreatedOption"))) {
	...
	@FindBy(xpath="//div[@class='pbBody']/table/tbody/tr")
	public List<WebElement> contactsCreatedDetails;
	
	public void verifyRecentlyCreatedContactsDetails(WebDriver driver) {
		if(WaitUtils.waitForListOfElementToBeVisible(driver, contactsCreatedDetails)) {
			for(WebElement eachRecentlyContactCreated:contactsCreatedDetails) {
				System.out.println(eachRecentlyContactCreated.getText()+" ");
			}
		}
	}
	...
	public boolean verifyCreatedViewNameDisplayedInDropDown(WebDriver driver,String expectedContactHomePageTitle) {
		...
		if(WaitUtils.waitForElementToBeVisible(driver, saveViewButton)) {
			...
			if(WaitUtils.waitForElementToBeVisible(driver, selectedOptionInDropDown)) {
				logger.info("Selected Default view Option Displayed in Contacts DropDown: "+ BaseTest.getSelectedOption(selectedOptionInDropDown));
			}
		}
...
//		if(uniqueViewName.getAttribute("value").contains(DataUtils.readContactsPageData("contact.uniqueviewName"))) {
//			isUniqueViewNameDisplayed=true;
//		}
	...
	public boolean verifyAccountNameDisplayedInAccountNameField(WebDriver driver) throws IOException {
		...
		String parentId=BaseTest.parentWindowId(driver);
		if(WaitUtils.waitForElementToBeVisible(driver, accountLookup)) {
			accountLookup.click();
			BaseTest.switchToChildWindow(driver, parentId);
