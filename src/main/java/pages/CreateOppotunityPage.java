...
//	public WebElement accountNameListed;
	...
	public WebElement saveButton;
	 ...
	 @FindBy(xpath="//div[@id='fchArea']/table/tbody/tr")
     public List<WebElement> opportunitiesPipelineTable;
	 
	 @FindBy(xpath="//div[@id='ep']//table/tbody/tr")
	 public List<WebElement> newOpportunityDetailsTable;
	 
	 @FindBy(xpath="//div[@id='fchArea']/table/tbody/tr")
	 public List<WebElement> stuckOpportunityDetailsTable;
	...
	public boolean verifyQuarterlySummaryLink(WebDriver driver,String interval,String includeOptions) {
		...
		if(quarlterlySummaryInterval.isDisplayed()) {
			BaseTest.scrollToElement(driver, quarlterlySummaryInterval);
			BaseTest.selectOption(quarlterlySummaryInterval,interval);
			logger.info(interval + "interval is selected");
		}
		if(includeOption.isDisplayed()) {
			BaseTest.scrollToElement(driver, includeOption);
			BaseTest.selectOption(includeOption, includeOptions);
	...
	public void verifyNewOpportunityDetails(WebDriver driver) {
		if(WaitUtils.waitForListOfElementToBeVisible(driver, newOpportunityDetailsTable)) {
			for(WebElement eachDetail:newOpportunityDetailsTable) {
				System.out.println(eachDetail.getText() + " ");
			}
		}System.out.println();
	}
	...
	public void enterNewOpportunityDetails(WebDriver driver) {
		...
		if(WaitUtils.elementToBeClickable(driver, searchOpportunityButton)) {
			...
			String parentWindowId=BaseTest.parentWindowId(driver);
			BaseTest.switchToChildWindow(driver, parentWindowId);
			...
			if(WaitUtils.waitForElementToBeVisible(driver, stageOption)) {
					BaseTest.selectOptionFromAllOptions(stageOption, "Needs Analysis");
			...
			if(WaitUtils.waitForElementToBeVisible(driver, leadSource)) {
				String leadSourceOption=BaseTest.selectOptionFromAllOptions(stageOption,"Web");
				logger.info(leadSourceOption + " lead source Option selected");
			}
			String parentWindow=BaseTest.parentWindowId(driver);
			if(WaitUtils.waitForElementToBeVisible(driver, primaryCampaignSearchButton)) {
				primaryCampaignSearchButton.click();
				BaseTest.switchToChildWindow(driver, parentWindow);
	...
	public boolean verifyStuckOpportunitiesPageIsDisplayed(WebDriver driver,String expectedStuckOpportunitiesPage ) {
		...
		if(WaitUtils.waitForElementToBeVisible(driver, stuckOpportunities)) {
			BaseTest.scrollToElement(driver, stuckOpportunities);
	...
	public void verifyStuckOpportunitiesDetails(WebDriver driver) {
		if(WaitUtils.waitForListOfElementToBeVisible(driver, stuckOpportunityDetailsTable)) {
			for(WebElement eachStuckOpportunity:stuckOpportunityDetailsTable) {
				System.out.println(eachStuckOpportunity.getText()+" ");
			}
		}
	}
	...
	public boolean verifyOpportunitiesDropDown(WebDriver driver) {
		...
		if(oppotunitiesDropDown.isDisplayed()) {
			BaseTest.listDropDown(oppotunitiesDropDown);
			isOpportunityDropDownDisplayed=true;
			//String firstSelectedOption=BaseTest.firstSelectedOption(oppotunitiesDropDown);
...
//	     }  
			List<WebElement> eachOpportunity=BaseTest.allOptions(oppotunitiesDropDown);
	...
	////a[contains(text(),'Opportunity Pipeline')]
            ...
            public boolean verifyTestOpportunityPipeLinePage(WebDriver driver,String expectedPipeLineTitle) {
            	...
            	if(WaitUtils.waitForElementToBeVisible(driver, opportunityPipeLineLink)) {
            		BaseTest.scrollToElement(driver, opportunityPipeLineLink);
            ...
            public void verifyPipeLineDetails(WebDriver driver) {
            	if(WaitUtils.waitForListOfElementToBeVisible(driver, opportunitiesPipelineTable)) {
            		for(WebElement eachPipeLineOpportunity:opportunitiesPipelineTable) {
            			System.out.println(eachPipeLineOpportunity.getText() + "  ");
            		}
            	}
